...
function coordinate(element){
  var between = (element.getBoundingClientRect().bottom - element.getBoundingClientRect().top)/2
  return between;
}
...
//create a button to add new sections and modify it CSS
  ...
  for (var variable of sectionsArray) {
    const newList=document.createElement('li');
    var sectionData=variable.getAttribute("data-nav");
    var sectionId=variable.getAttribute("id");
...
document.addEventListener("scroll",function(){
  ...
  for (let variable of sections) {
   ...
   let optimalPosition = coordinate(variable);
   // check if the current section is in the viewpoint
  if (variable.getBoundingClientRect().top<optimalPosition&&variable.getBoundingClientRect().top>=-(optimalPosition)){
...
//add a new list in the navigation bar
  const newList=document.createElement('li');
  newList.style.margin = "10px";
  var sectionData=sectionsArray[sectionsArray.length-1].getAttribute("data-nav");
  var sectionId=sectionsArray[sectionsArray.length-1].getAttribute("id");
  newList.innerHTML=`<a data-list=${sectionId}>${sectionData}</a>`;
